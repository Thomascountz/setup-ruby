...
* If there is a `Gemfile.lock` file (or `$BUNDLE_GEMFILE.lock`, `$BUNDLE_LOCKFILE`, or `gems.locked`) with a `BUNDLED WITH` section,
...
If there is a `Gemfile.lock` (or `$BUNDLE_GEMFILE.lock`, `$BUNDLE_LOCKFILE`, or `gems.locked`), `bundle config --local deployment true` is used.
